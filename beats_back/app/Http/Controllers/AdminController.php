...
public function fetchAllHighScores()
{
    try {
        // Retrieve high score data from Firebase
        $allHighScoresRef = $this->database->getReference('users')->getSnapshot()->getValue();

        // Initialize totals for each game mode
        $totalShapes = 0;
        $totalLetters = 0;
        $totalNumbers = 0;
        ...
        if ($allHighScoresRef) {
            foreach ($allHighScoresRef as $scores) {
                // Accumulate only if score is defined and numeric
                $totalShapes += isset($scores['shapesHighScore']) && is_numeric($scores['shapesHighScore']) ? $scores['shapesHighScore'] : 0;
                $totalLetters += isset($scores['lettersHighScore']) && is_numeric($scores['lettersHighScore']) ? $scores['lettersHighScore'] : 0;
                $totalNumbers += isset($scores['numbersHighScore']) && is_numeric($scores['numbersHighScore']) ? $scores['numbersHighScore'] : 0;
            }
        }

        // Return the summed scores in JSON format
